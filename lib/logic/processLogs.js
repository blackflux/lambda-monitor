...
const promiseComplete = require("./util/promiseComplete");
...
module.exports = (event, context, callback, rb) => zlibPromise
  ...
  .then(([resultParsed, toLog]) => {
    ...
    return promiseComplete([
      timeoutPromise(logz.log(process.env.STAGE, toLog), timeout, 'logz'),
      timeoutPromise(loggly.log(process.env.STAGE, toLog), timeout, 'loggly'),
      timeoutPromise(datadog.log(process.env.STAGE, toLog), timeout, 'datadog')
    ]).then(() => resultParsed);
  })
